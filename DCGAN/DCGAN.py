...
import os
import argparse
OBfrom ast import literal_eval
...
class DCGAN:
    ...
    def train(self, epochs, image_path, batch_size=32, save_interval=50):
        ...
        for epoch in range(epochs):
            ...
            print(str(epoch)+" [D loss: "+str(d_loss[0])+" | D Accuracy: "+str(100 * d_loss[1])+"] [G loss: "+str(g_loss)+"]")
    ...
    def save_imgs(self, epoch):
        ...
        for i, img_array in enumerate(imgs):
            ...
            if not os.path.exists(path):
                os.makedirs(path)  
            imsave(path+"/"+str(epoch)+"_"+str(i)+".png",img_array)    
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', help='Path to directory of images of correct dimensions, using *.[filetype] (e.g. *.png) to reference images', default="/home/b16004/wd/Dataset/Art_128x128/Landscape_128x128/*.png")
    ...
    parser.add_argument('--image_size', help='Size of images as tuple (height,width). Height and width must both be divisible by (2^5)', default="(128, 128)")
    parser.add_argument('--epochs', help='Number of epochs to train for', default=500000)
    parser.add_argument('--save_interval', help='How many epochs to go between saves/outputs', default=500)
